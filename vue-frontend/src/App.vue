<template>
  <div class="wrapper">
    ...
    <nav class="navbar" role="navigation" aria-label="main navigation">
      ...
      <div id="navbarBasicExample" class="navbar-menu" v-bind:class="{'is-active': showMobileMenu}">
        ...
        <div class="navbar-start">
          ...
          <form method="GET" action="/search">
            <div class="field has-addons">
              <div class="control">
                <input class="input" type="text" placeholder="Find Tutorial" name="query" style="margin-top:8px">
              </div>
        ...
        <div class="navbar-end">
          ...
          <router-link to="/tutorial" class="button is-info">Tutorial</router-link>
              <template v-if="$store.state.isAuthenticated">
                  <router-link to="/account" class="button is-light">Account</router-link>
              </template>
               <template v-if="$store.state.isAuthenticated">
                  <router-link to="/my-list" class="button is-light">My tutorial</router-link>
              </template>
    ...
    <!-- footer -->
    <footer class="footer">
      <p class="has-text-centered">Test Case</p>
    </footer>
  ...
  computed: {
    cartTotalLength() {
      ...
      for (let i = 0; i < this.cart.items.length; i++) {
        totalLength += this.cart.items[i].product.quantity
      }
